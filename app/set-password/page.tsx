...
if (!supabase) {
setMessage("Add Supabase keys to enable account setup.");
setChecking(false);
return;
}

async function completeAuthFromLink() {
if (!supabase) {
return;
}
...
const supabase = supabaseBrowser();
if (!supabase) {
setMessage("Account setup is currently unavailable.");
return;
}
...
<h1 className="mt-2 text-3xl font-black text-legacy-ink">Welcome! Set your password</h1>
<p className="mt-2 text-legacy-muted">Create a password to activate your client portal account.</p>
</div>

{done ? (
<div className="grid gap-3">
<div className="rounded-lg bg-legacy-lavender p-3 text-sm text-legacy-plum">
...
<p className="text-legacy-muted">Checking your invitation link...</p>
) : ready ? (
...
{message ? (
<p className="rounded-lg bg-legacy-lavender p-3 text-sm text-legacy-plum">{message}</p>
) : null}
</form>
) : (
<div className="grid gap-3">
<p className="rounded-lg bg-legacy-lavender p-3 text-sm text-legacy-plum">
{message || "This invitation link is invalid or has expired. Please ask Lucille's Legacy to send you a new one."}
</p>
