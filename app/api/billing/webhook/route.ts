...
if (event.type === "checkout.session.completed") {
  ...
  if (clientId && invoiceIds.length > 0) {
    ...
    .from("invoices")
    .select("id, label, amount_cents, amount_paid_cents")
    .in("id", invoiceIds);
  ...
  for (const invoice of invoices) {
    ...
    await admin.from("client_timeline").insert({
      ...
      title: `Payment received: ${invoice.label}`,
      description: `Status changed to ${status}.`,
      metadata: { invoice_id: invoice.id, status, stripe_payment_intent_id: paymentIntentId }
    });

    await admin.from("notifications").insert({
      client_id: clientId,
      title: "Payment received",
      body: `We received your payment for ${invoice.label}. Thank you!`,
      kind: "payment_received"
    });

    await notifyClientByEmail(clientId, "PAYMENT_CONFIRMATION", { label: invoice.label });
  }
  }
}
...
if (event.type === "charge.refunded") {
  ...
  if (paymentIntentId) {
    ...
    .maybeSingle();
    const clientId = paymentResult.data?.client_id || invoiceResult.data?.client_id;
    const label = invoiceResult.data?.label || "your invoice";
    if (clientId) {
      await admin.from("client_timeline").insert({
        client_id: clientId,
        event_type: "invoice_status_changed",
        title: `Refund issued: ${label}`,
        description: "Status changed to Refunded.",
        metadata: { stripe_payment_intent_id: paymentIntentId }
      });
      await admin.from("notifications").insert({
        client_id: clientId,
        title: "Refund issued",
        body: `A refund was issued for ${label}.`,
        kind: "invoice_ready"
      });
    }
  }
